...
/datum/category_item/player_setup_item/general/body/proc/SetSpecies(mob/user)
	...
	if(!(current_species.spawn_flags & SPECIES_CAN_JOIN))
		...
		restricted = 1
	else if(current_species.ckeyrestricted)
		if(!(current_species.ckeyrestricted == usr.ckey))
			restricted = 3
	...
	if(restricted)
		if(restricted == 1)
			...
			dat += "<font color='red'><b>You cannot play as this species.</br><small>This species is only available to a certain donator.</small></b></font></br>"
